/*
  # Initial Database Setup for SGED (Sistema de Gestión de Eventos Deportivos)

  ## Tables Created
  
  1. **tournament_config**
    - Stores tournament metadata (name, organization, dates, team lists)
    - Default values for Copa Dorada 2024
  
  2. **matches**
    - Individual match records with scores, dates, venues
    - Links to auth.users for tracking updates
    - Supports incomparecencia (no-show) tracking
  
  3. **standings**
    - Team standings with all FVF statistics
    - Auto-calculated from match results
    - Unique constraint on (grupo, equipo)
  
  4. **audit_log**
    - Tracks all changes for compliance
    - Stores user actions and IP addresses
  ...
  - Only admins can edit matches and configuration
  - Users can only view their own roles
  - Audit logs restricted to admins
  
  ## Sample Data
  
  - Tournament configuration with 3 groups (A, B, C)
  - Sample matches for groups A and B, Jornada 1
